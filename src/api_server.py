...
from agents.tools.taxonomy_tools import classify_causality_tool  # noqa: E402
...
def classify_causality_on_reasoning_chain(reasoning_chain: list[str], raw_response: str) -> dict:
    """
    Classify the causality type based on the Reasoner's reasoning chain.
    
    This is done AFTER the Reasoner generates its arguments, using the
    reasoning chain itself as input (not the original claim).
    
    Args:
        reasoning_chain: The reasoning chain generated by the Reasoner
        raw_response: The full raw response from the Reasoner
        
    Returns:
        Causality classification dict for use by CounterReasoner
    """
    # Build a text representation of the reasoning chain
    chain_text = "\n".join(reasoning_chain) if reasoning_chain else raw_response[:2000]
    
    print(f"\n{'─'*70}")
    print("🔬 CLASSIFYING CAUSALITY on reasoning chain...")
    print(f"{'─'*70}")
    print(f"   📝 Chain length: {len(chain_text)} chars")
    
    try:
        # Use the classify_causality_tool with the reasoning chain as "claim"
        causality = classify_causality_tool.invoke({
            "claim": chain_text,
            "context": "This is a legal reasoning chain generated by a Reasoner agent analyzing a legal claim."
        })
        
        causality_type = causality.get("causality_type", "Unknown")
        print(f"   ✅ Causality classified: {causality_type}")
        print(f"   📋 Warrant: {causality.get('warrant', {}).get('denominazione', 'N/A')}")
        
        return causality
        
    except Exception as e:
        print(f"   ⚠️ Causality classification failed: {e}")
        return {
...
def pipeline():
    ...
    try:
        ...
        # STEP 2: Classify causality on the reasoning chain (NOT on the claim)
        causality = classify_causality_on_reasoning_chain(
            reasoner_result.reasoning_chain,
            reasoner_result.raw_response
        )
        
        # Update the reasoner result with the classified causality
        reasoner_result.causality_classification = causality

        # STEP 3: Counter-Reasoner (receives AGAINST articles/precedents + causality from chain)
        print(f"\n{'─'*70}")
        print("⚔️  STEP 3: Esecuzione Counter-Reasoner (con articoli CONTRO)...")
        print(f"{'─'*70}")
        print(
            f"   📚 Knowledge base: {len(against_statutes)} statuti, {len(against_precedents)} precedenti"
        )
        print(f"   🎯 Causalità da attaccare: {causality.get('causality_type', 'N/A')}")
