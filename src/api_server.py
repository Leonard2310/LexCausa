...
def classify_causality_on_reasoning_chain(
    reasoning_chain: list[str], raw_response: str
) -> dict:
    """
    ...
    try:
        ...
        print(
            f"   📋 Warrant: {causality.get('warrant', {}).get('denominazione', 'N/A')}"
        )
        ...
        return {"causality_type": "Unknown", "error": str(e)}
...
def pipeline():
    ...
    try:
        ...
        causality = classify_causality_on_reasoning_chain(
            reasoner_result.reasoning_chain, reasoner_result.raw_response
        )
